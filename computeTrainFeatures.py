...
def main():
    ...
    parser.add_argument('-l', '--labels', default='data/dev.answers',
                        help='input file (data/dev.answers)')
    ...
    def sentences():
        with open(opts.input) as refHypo,open(opts.labels) as labels:
    ...
    feature = []
    labels = numpy.empty((opts.num_sentences,))
    ...
    json_decoded['labels'] = labels.tolist()
    json_file = open("trainFeatures.json", 'w')
